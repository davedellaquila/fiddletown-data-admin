...
(function() {
  ...
  function filterEventsByDateRange(events, fromDate, toDate) {
    ...
    return events.filter(ev => {
      ...
      // 2. Have no dates (undated events)
      // Note: We exclude events that start before fromDate even if they span across it
      if (normalizedFromDate) {
        ...
        if (startDate && startDate < normalizedFromDate) {
          // Starts before fromDate - exclude it (even if it spans across fromDate)
          return false;
        }
        // Has endDate but no startDate - exclude (we need a start date to filter properly)
        if (!startDate && endDate) {
          return false;
        }
      }
