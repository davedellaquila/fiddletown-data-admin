...
|------|------|-------|---------|---------------|
| 2026-06-30 | Dev | Events widget desktop filter row | Rebalanced the desktop widget filter toolbar into four measured grid columns so Date Presets, View, Group by, and Keywords (including selected count) fit on one row without oversized controls. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page after deploy |
