...
|------|------|-------|---------|---------------|
| 2026-07-08 | Dev | Events widget To clear follow-up | Fixed the inline To-date clear path to cancel pending date rerenders, read the visible From date at click time, explicitly clear To, and rerender open-ended from From. Browser validation verified To value/attribute clear to blank, results reopen from From onward, and From-after-To still clamps To to From. | Dave — hard-refresh events page and verify To clear on the live widget |
| 2026-07-08 | Dev | Events widget date clamp/clear | Changed widget date normalization so moving From after To clamps To to From and rerenders immediately; the inline To-date clear button now leaves To blank and rerenders open-ended from From. Local browser validation verified 07/04-07/05, From -> 07/10 clamps To to 07/10 (1 event), then clear To shows 20 events from 07/10 onward with no console warnings/errors. | Dave — hard-refresh events page and verify From/To date behavior |
| 2026-07-07 | Dev | Events widget Calendar switch anchor | Fixed Calendar View switches to use the live widget state/visible From date, cancel pending date-input rerenders before switching layouts, and re-anchor Calendar date changes to the matching From month. Local browser validation verified From `2026-09-19` opens Calendar View at September 2026 with no console warnings/errors. | Dave — hard-refresh events page and verify Calendar View lands on the From month |
| 2026-07-07 | Dev | Vercel gh-pages preview failure | Added `web/public/web/vercel.json` so the normal Vite build publishes `gh-pages:/web/vercel.json`; this lets Vercel read `git.deploymentEnabled.gh-pages=false` instead of failing because the configured `web` root directory is missing on the generated static branch. | Dave — ignore the prior failed Vercel preview; verify future `gh-pages` updates are skipped/ignored by Vercel |
| 2026-07-07 | Dev | Events widget calendar month anchor | Changed Calendar View transitions so switching into Calendar View scrolls to the calendar month matching the active From date, and ensured the calendar renders the From month even when the first matching event is later. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify Calendar View starts at the From month |
| 2026-07-06 | Dev | Events widget date input filtering | Fixed date input synchronization so From/To input edits debounce-rerender the event list immediately instead of waiting for a later change/blur event. This keeps the visible To date aligned with the actual date-filtered results. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify To date bounds the visible events |
| 2026-07-06 | Dev | Events widget clear date action | Fixed the inline compact widget date clear action so tapping/clicking the X explicitly clears the To date, keeps it cleared through rerender, and wires pointer/touch/click events for mobile Safari. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify the To date clears |
| 2026-07-06 | Dev | Events widget clear date placement | Moved the compact widget date clear control from the label row to the right side of the To date input row and updated date grids across breakpoints so the sticky panel can be shorter without a top-row X. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify compact sticky panel |
| 2026-07-05 | Dev | Events widget mobile date field follow-up | Adjusted the small compact sticky date fields after iPhone QA: restored readable date text sizing/line-height, kept the WebKit calendar affordance hidden to avoid width overflow, and reduced the clear-button column so the top-right control no longer crowds the panel edge. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify iPhone widget |
| 2026-07-05 | Dev | Events widget small date fields | Tightened the small compact sticky widget date input rendering by making date fields shrinkable, centering their value text, and hiding the WebKit calendar affordance inside the compact filter shell so the date row no longer pushes past the visible widget width. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify small widget |
| 2026-07-04 | Dev | Amador Farmers Market dates | Corrected the published Farmers Market data model by archiving the four broad season-span rows and publishing 57 dated occurrence rows: Sutter Creek Saturdays 8:30-11:30, Ione Wednesdays 5-8, Plymouth Thursdays 5-8, and Volcano first/third Sundays 9-12. Public read-back verifies July 5 and July 19 Volcano rows exist while July 12 does not. | Dave — hard-refresh events page after session cache clears |
| 2026-07-04 | Dev | Events widget sticky filter shell | Strengthened compact sticky shell overrides so the date row and filter menu row share one enclosing background/border instead of rendering as separate row panels. Source/public widget copies match and JS syntax checks pass. | Dave — hard-refresh events page and verify sticky filter panel |
| 2026-07-04 | Dev | Krissie Vineyards July 4 event | Published Celebrate America's 250th Birthday and Freedom at Krissie Vineyards (July 3-5, 2026, 11 AM-5 PM) with repo-hosted poster asset and keywords: amador wine, food, holiday, wine event, winery. Supabase read-back verifies the published row and keyword relationships. | Dave — refresh widget/admin after deploy |
| 2026-07-03 | Dev | Events widget mobile sticky filters | Added mobile-specific compact sticky shell overrides so the date inputs stay visible/bounded, filter menus stay in a 2x2 grid, and the summary row remains compact inside the shared sticky background. Source/public widget copies match and JS syntax checks pass; local Chrome visual smoke is blocked by Chrome crash. | Dave — hard-refresh events page and verify on iPhone |
