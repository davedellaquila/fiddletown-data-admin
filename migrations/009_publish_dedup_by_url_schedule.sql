-- Migration: Prevent duplicate published events for the same listing + schedule
-- Matches public widget dedupe: same normalized URL + date/time slot = same event.

CREATE OR REPLACE FUNCTION public.normalize_event_url(input text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    ELSE lower(
      regexp_replace(
        regexp_replace(
          regexp_replace(trim(input), '^https?://', '', 'i'),
          '^www\.', '', 'i'
        ),
        '/+$', ''
      )
...
CREATE OR REPLACE FUNCTION public.events_share_schedule(
  a_start_date date,
  a_end_date date,
  a_start_time time,
  a_end_time time,
...
IMMUTABLE
AS $$
  SELECT a_start_date IS NOT DISTINCT FROM b_start_date
     AND coalesce(a_end_date, a_start_date) IS NOT DISTINCT FROM coalesce(b_end_date, b_start_date)
     AND coalesce(to_char(a_start_time, 'HH24:MI'), '') = coalesce(to_char(b_start_time, 'HH24:MI'), '')
     AND coalesce(to_char(a_end_time, 'HH24:MI'), '') = coalesce(to_char(b_end_time, 'HH24:MI'), '');
$$;
