...
(function() {
  ...
  async function fetchEvents({ url, key, from = null, to = null, limit = 200 }) {
      ...
      api.searchParams.set('select','id,name,slug,host_org,start_date,end_date,location,website_url,image_url,recurrence,sort_order');
  ...
  function renderListLayout(events) {
    ...
    months.forEach(month => {
      ...
      grouped[month].forEach((event, idx) => {
        ...
        // Add image icon if image exists
        if (hasImage) {
          html += `<span class="ssa-image-icon" data-event-id="${eventId}" title="Hover to preview image">🖼️</span>`;
        }
        
        // Event name as clickable link
        html += `<a href="${eventUrl}" class="ssa-event-link" ${event.website_url ? 'target="_blank" rel="noopener"' : ''}>`;
        ...
        if (event.keywords && event.keywords.length > 0) {
          html += ` <span class="ssa-keywords-inline">${event.keywords.map(kw => `<span class="ssa-tag">${kw}</span>`).join('')}</span>`;
        }
        html += `</li>`;
      });
  ...
  function renderGridLayout(events) {
    if (events.length === 0) {
      return '<div class="ssa-empty">No events found.</div>';
    }
    ...
    const cards = events.map(ev => `
      <article class="ssa-card">
        ...
        ${ev.keywords && ev.keywords.length > 0 ? `<p class="ssa-keywords">${ev.keywords.map(kw => `<span class="ssa-tag">${kw}</span>`).join('')}</p>` : ''}
  ...
  async function renderEvents(mount, rows, state) {
    ...
    if (layout === LAYOUTS.LIST) {
      eventsHTML = renderListLayout(filteredRows);
    } else if (layout === LAYOUTS.GRID) {
      eventsHTML = renderGridLayout(filteredRows);
    }
  ...
  function attachEventHandlers(mount, rows, state) {
    ...
    if (state.layout === LAYOUTS.LIST) {
      ...
      mount.querySelectorAll('.ssa-image-icon').forEach(icon => {
        const eventId = icon.dataset.eventId;
        const eventItem = mount.querySelector(`[data-event-id="${eventId}"]`);
        if (!eventItem) return;
        
        // Get image URL from data attribute
        const imageUrl = eventItem.dataset.eventImage;
      ...
      document.addEventListener('mouseenter', function(e) {
        if (e.target.closest('.ssa-image-preview')) {
          if (hoverTimeout) clearTimeout(hoverTimeout);
        }
      }, true);
  ...
  function injectStyles() {
    ...
    css.textContent = `
      ...
      .ssa-link{text-decoration:underline;color:#3b82f6}
    `;
